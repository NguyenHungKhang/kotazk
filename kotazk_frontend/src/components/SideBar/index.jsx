...
const SideBar = ({ open, setOpen }) => {
    ...
    const isDashboard = /^\/workspace$/.test(pathname);
            ...
            <Box bgcolor={alpha("#f5f5fc", 0.07)} height="100%" p={2} borderRadius={4}>
                ...
                >
                    ...
                    {isDashboard ?
                        ...
                        : <WorkspaceProjectSideBar open={open} setOpen={setOpen} />}
                </Stack>
            </Box>
        </Box>
    );
...
const WorkspaceProjectSideBar = ({ open, setOpen }) => {
    const theme = useTheme();
...
const DashboardSideBar = ({ open, setOpen }) => {
    const theme = useTheme();
    const { projectId } = useParams();
    const workspaceList = useSelector((state) => state.workspace.currentWorkspaceList);
    const pathname = window.location.pathname;
    ...
    const initialFetch = async () => {
        ...
        await apiService.workspaceAPI.getPageSumary(data)
            .then(res => { dispatch(setCurrentWorkspaceList(res.data.content)); })
            .catch(err => console.warn(err))
    }
    ...
    return (
        <List sx={{ width: '100%' }}>
            {workspaceList?.content?.map((workspace, index) => (
                <ListItem
                    key={index}
                    sx={{
                        py: 0.5,
                        mt: 1,
                        borderRadius: 1,
                        bgcolor: projectId == workspace.id ? theme.palette.primary.main : 'transparent',
                        '&:hover': {
                            bgcolor: alpha(theme.palette.primary.main, 0.5),
                        }
                    }}
                ...
                >
                    ...
                    {open && (
                        <ListItemText
                            primary={
                                <Typography noWrap color={projectId == workspace.id ? theme.palette.getContrastText(theme.palette.primary.main) : theme.palette.text.primary}>
                                    {workspace.name}
                                </Typography>
                            }
                            sx={{ ml: 2 }}
